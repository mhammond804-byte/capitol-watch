    ...
    try:
        ...
        for bill in sponsored:
            ...
            if typ and num and congress:
                ...
                bills_to_check.add((key, congress, typ, num, title))
    except Exception as e:
        print(f"  {name} ({bid}): ERROR - {e}")

print(f"\nTotal unique bills found: {len(bills_to_check)}")

# Find uncached bills
uncached = []
for key, cong, typ, num, title in bills_to_check:
    if key not in cache:
        uncached.append((key, cong, typ, num, title))
